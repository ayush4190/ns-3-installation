...
main (int argc, char *argv[])
{
  ...
  std::string deviceName ("eno1"); // your machine name(use ifconfig)
  std::string remote ("74.125.24.103"); // google.com(you can use any other website)
  ...
  Ipv4Address localIp ("10.100.14.231"); //change it to your ip address
  NS_ABORT_MSG_IF (localIp == "1.2.3.4", "You must change the local IP address before running this example");

  Ipv4Mask localMask ("255.255.252.0"); //change it with your subnet mask
  ...
  // "1.2.3.4" string.
  //
  Ipv4Address gateway ("10.100.12.1"); // change it t your gateway(netstat -rn)
